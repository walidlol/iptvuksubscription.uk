...
function TvIcon() {
  return (
    <svg className="w-12 h-12 text-text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {DEVICES.map((category) => (
            <div
              key={category.title}
              className="flex flex-col items-center text-center p-6 rounded-xl border border-border bg-bg-surface/50 hover:bg-bg-elevated/50 transition-colors"
            >
              {/* Icon */}
              <div className="mb-4 p-4 rounded-full bg-bg-elevated/50">
                {category.icon}
              </div>
